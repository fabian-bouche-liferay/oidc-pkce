package com.liferay.samples.fbo.openid.pkce;
...
public class PKCEOpenIdConnectAuthenticationHandlerImpl
	...
	/* START Replacing OpenIdConnectTokenRequestUtil to include PKCE */
	private OIDCTokens request(
			...
			String tokenRequestParametersJSON, CodeVerifier codeVerifier)
		throws Exception {

		AuthorizationGrant authorizationCodeGrant = new AuthorizationCodeGrant(
			authenticationSuccessResponse.getAuthorizationCode(), redirectURI, codeVerifier);
	...
	/* END Replacing OpenIdConnectTokenRequestUtil to include PKCE */
	...
	public void processAuthenticationResponse(
		...
		// Read codeVerifier in session
		CodeVerifier codeVerifier = (CodeVerifier) httpSession.getAttribute(OIDC_CODE_VERIFIER);
		...
		OIDCTokens oidcTokens = request(
			...
			oAuthClientEntry.getTokenRequestParametersJSON(), codeVerifier);
	...
	private URI _getAuthenticationRequestURI(
		...
		builder = builder.endpointURI(
			...
			(State)runtimeRequestParameters.get("state")
		// Build request with code challenge
		).codeChallenge(
				(CodeChallenge)runtimeRequestParameters.get("codeChallenge"), 
